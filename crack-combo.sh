...
if [ "$status_timer_input" = "y" ]; then
    hashcat.exe --session="$session" --status --status-timer=2 --increment --increment-min="$min_length" --increment-max="$max_length" -m 22000 hash.hc22000 -a 6 -w 4 --outfile-format=2 -o plaintext.txt "$wordlist_path/$wordlist" "$mask"
else
